...
app.controller('FoodController',
    function FoodController($scope, $ionicLoading, $ionicModal, $ionicSlideBoxDelegate, $localStorage, eventData, LocationService){
        $ionicLoading.show({
            content:'Loading',
            animation:'fade-in',
            ShowBackDrop: true,
        ...
        if($scope.location === 'Getting location...') {
            $scope.$watch('location', function (newLocation, oldLocation) {
                if (typeof newLocation === "object") {
                    ...
                    eventData.getData(newLocation, "food", $scope.selectedDate).
                        then(function (data) {
                            if (data) {
