...
app.controller('SportsController',
    function SportsController($scope, $ionicLoading, $ionicModal, $ionicSlideBoxDelegate, $localStorage, eventData, LocationService){
        $ionicLoading.show({
            content:'Loading',
            animation:'fade-in',
            ShowBackDrop: true,
        ...
        if($scope.location === 'Getting location...') {
            $scope.$watch('location', function (newLocation, oldLocation) {
                if (typeof newLocation === "object") {
                    ...
                    eventData.getData(newLocation, "sports", $scope.selectedDate).
                        then(function (data) {
                            if (data) {
